...
`My.Computer.FileSystem` 对象提供用于处理文件和文件夹的工具。  它的属性、方法和事件使您可以创建、复制、移动、调查和删除文件和文件夹。  `My.Computer.FileSystem` 的性能优于由 [!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] 提供的用于向后兼容的旧式功能（例如，`FileOpen`、`FileClose`、`Input`、`InputString`、`LineInput` 等）的性能。  

...
6.  若事件调用某个已在堆栈中的事件过程，则确保未通过触发该事件导致“事件级联”。  事件级联与未终止的递归过程调用相似，但是它不明显，因为该调用是通过 [!INCLUDE[vbprvb](../../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] 发出的，而不是代码中的显式调用。  使用 `Calls` 对话框可查看堆栈中有哪些活动的过程。  

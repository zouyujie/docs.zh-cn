...
`Sub` 过程是包含在 `Sub` 语句和 `End Sub` 语句中的一系列 [!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] 语句。  `Sub` 过程执行一项任务，再将控制权返回给调用代码，但是不将值返回给调用代码。  

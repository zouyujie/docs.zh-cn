...
使用嵌入式表达式可以创建包含在运行时计算的表达式的 XML 文本。  嵌入式表达式的语法是 `<%=` `expression` `%>`，该语法与 [!INCLUDE[vstecasp](../../../../csharp/language-reference/preprocessor-directives/includes/vstecasp-md.md)] 中使用的语法相同。  
...
## 限定规则的应用范围  
 编译器将每个 XML 文本都转换为对相应文本类型的构造函数的调用。  XML 文本中的文本内容和嵌入式表达式作为参数传递给构造函数。  这意味着，可在 XML 文本中使用的所有 [!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] 编程元素也都可在其嵌入式表达式中使用。  

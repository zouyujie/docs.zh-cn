---
title: "Visual Basic 中的泛型类型 (Visual Basic) | Microsoft Docs"
ms.custom: ""
ms.date: "12/15/2016"
...
作为通用工具的螺丝刀  
 ...
 有关使用泛型类型的更多信息，请参阅[如何：使用泛型类](../../../../visual-basic/programming-guide/language-features/data-types/how-to-use-a-generic-class.md)。  
...
## 合格的编程元素  
 你可以定义并使用泛型类、结构、接口、过程和委托。 请注意，[!INCLUDE[dnprdnshort](../../../../csharp/getting-started/includes/dnprdnshort_md.md)] 定义了几个代表常用泛型元素的泛型类、结构和接口。<xref:System.Collections.Generic?displayProperty=fullName> 命名空间提供字典、列表、队列和堆栈。 在定义自己的泛型元素之前，请查看 <xref:System.Collections.Generic?displayProperty=fullName> 中是否已提供了此元素。  
...
-   类型实参必须是*引用类型*或*值类型*  
 ...
 如果需要强制实施多个要求，则可以使用以逗号分隔的*约束列表*（括在大括号 \(`{ }`\) 内）。 若需要可访问的构造函数，请在列表中加入 [New 运算符](../../../../visual-basic/language-reference/operators/new-operator.md) 关键字。 若需要引用类型，请加入 `Class` 关键字；若需要值类型，请加入 `Structure` 关键字。  

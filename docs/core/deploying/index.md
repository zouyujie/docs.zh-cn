...
1. 为项目创建目录，然后在命令栏中，键入 `dotnet new console` 创建新的 C# 控制台项目。
...
若要创建内存占用较小的独立部署，请从按照创建独立部署的前两个步骤着手。 运行 `dotnet new console` 命令并在向应用添加 C# 源代码后，执行以下操作：

---
title: "LINQ to XML 安全性 (C#) | Microsoft Docs"
ms.custom: 
ms.date: 2015-07-20
ms.prod: .net
ms.reviewer: 
...
## <a name="linq-to-xml-security-overview"></a>LINQ to XML 安全性概述  
 ...
 如果必须处理来自未知源的不受信任的数据，Microsoft 建议使用 <xref:System.Xml.XmlReader> 类的一个实例，并将该实例配置为筛选出已知 XML 拒绝服务 (DoS) 攻击。  
  
 如果已配置 <xref:System.Xml.XmlReader> 以缓解拒绝服务攻击，则可以使用该读取器填充 LINQ to XML 树，同时仍受益于 LINQ to XML 所提供的高编程效率。 许多缓解技术均涉及创建配置为可缓解安全问题的读取器，然后通过配置的读取器实例化 XML 树。  
...
## <a name="mitigation-of-xml-xsd-xpath-and-xslt-attacks"></a>缓解 XML、XSD、XPath 和 XSLT 攻击  
 LINQ to XML 是基于 <xref:System.Xml.XmlReader> 和 <xref:System.Xml.XmlWriter> 生成的。 LINQ to XML 通过 <xref:System.Xml.Schema?displayProperty=fullName> 和 <xref:System.Xml.XPath?displayProperty=fullName> 命名空间中的扩展方法支持 XSD 和 XPath。 将 <xref:System.Xml.XmlReader>、<xref:System.Xml.XPath.XPathNavigator> 和 <xref:System.Xml.XmlWriter> 与 LINQ to XML 结合使用，可以调用 XSLT 来转换 XML 树。  
  
 如果在较不安全的环境中操作，会出现与 XML 相关联的问题和在 <xref:System.Xml?displayProperty=fullName>、<xref:System.Xml.Schema?displayProperty=fullName>、<xref:System.Xml.XPath?displayProperty=fullName> 和 <xref:System.Xml.Xsl?displayProperty=fullName> 中使用类的问题。 这些问题包括但不限于以下问题：  
...
-   不接受来自不受信任的程序集的支持组件，例如 <xref:System.Xml.NameTable>、<xref:System.Xml.XmlNamespaceManager> 和 <xref:System.Xml.XmlResolver> 对象。  
...
### <a name="do-not-call-codeaccesspermissionsassert-in-an-event-handler"></a>不要在事件处理程序中调用 CodeAccessPermissions.Assert  
 ...
 如果具有较高权限的程序集中的代码在事件处理程序中调用 <xref:System.Security.CodeAccessPermission.Assert%2A?displayProperty=fullName>，然后将 XML 树传递给具有受限权限的恶意程序集，则恶意程序集可能引发事件。 由于该事件运行具有较高权限的程序集中的代码，因此恶意程序集将使用提升的特权运行。  
  
 Microsoft 建议你不要在事件处理程序中调用 <xref:System.Security.CodeAccessPermission.Assert%2A?displayProperty=fullName>。  
...
### <a name="avoid-excessive-buffer-allocation"></a>避免分配过多的缓冲区  
 ...
 如果恶意用户提交或上载非常大的 XML 文档，则可能导致 LINQ to XML 使用过多的系统资源。 这可能构成拒绝服务攻击。 为防止这种情况发生，可以设置 <xref:System.Xml.XmlReaderSettings.MaxCharactersInDocument%2A?displayProperty=fullName> 属性并创建一个读取器，然后限制其可加载的文档大小。 然后使用该读取器创建 XML 树。  
  
 例如，如果你知道来自不受信任源的 XML 文档的预计最大大小不会超过 50K 字节，则将 <xref:System.Xml.XmlReaderSettings.MaxCharactersInDocument%2A?displayProperty=fullName> 设置为 100,000。 这不会妨碍您处理 XML 文档，同时还可在要上载的文档会使用大量内存的情况下缓解拒绝服务威胁。  
  
### <a name="avoid-excess-entity-expansion"></a>避免过度扩展实体  
 使用 DTD 时的一种已知拒绝服务攻击是导致过度扩展实体的文档造成的。 为防止这种情况发生，可以设置 <xref:System.Xml.XmlReaderSettings.MaxCharactersFromEntities%2A?displayProperty=fullName> 属性并创建一个读取器，然后限制从实体扩展中生成的字符数。 然后使用该读取器创建 XML 树。  
  
### <a name="limit-the-depth-of-the-xml-hierarchy"></a>限制 XML 层次结构的深度  
 在提交具有过深层次结构的文档时，会发生一种可能的拒绝服务攻击。 为了防止这种情况发生，可以将 <xref:System.Xml.XmlReader> 包装在你自己的可计算元素深度的类中。 如果深度超过预设的合理等级，您可以终止处理恶意文档。  
  
### <a name="protect-against-untrusted-xmlreader-or-xmlwriter-implementations"></a>防止不受信任的 XmlReader 或 XmlWriter 实现  
 管理员应验证任何外部提供的 <xref:System.Xml.XmlReader> 或 <xref:System.Xml.XmlWriter> 实现都具有强名称且已经在计算机配置中注册。 这可防止加载伪装成读取器或编写器的恶意代码。  
  
### <a name="periodically-free-objects-that-reference-xname"></a>定期释放引用 XName 的对象  
 为防止某些类型的攻击，应用程序程序员应该定期释放引用应用程序域中 <xref:System.Xml.Linq.XName> 对象的所有对象。  

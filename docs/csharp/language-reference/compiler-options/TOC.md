...
# [/target:appcontainerexe（创建 Windows 可执行文件）](target-appcontainerexe-compiler-option.md)
...
# [/subsystemversion（指定最低子系统版本）](subsystemversion-compiler-option.md)
...
# [/target:winmdobj（为 Windows 运行时创建中间文件）](target-winmdobj-compiler-option.md)

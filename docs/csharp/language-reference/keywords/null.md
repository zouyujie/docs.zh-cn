---
title: "null（C# 参考） | Microsoft Docs"
ms.custom: ""
ms.date: "12/03/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "devlang-csharp"
ms.tgt_pltfrm: ""
ms.topic: "article"
f1_keywords: 
  - "null"
  - "null_CSharpKeyword"
dev_langs: 
  - "CSharp"
helpviewer_keywords: 
  - "null 关键字 [C#]"
ms.assetid: fecb1c60-6232-4efe-87f7-9a86ba2e9119
caps.latest.revision: 17
caps.handback.revision: 17
author: "BillWagner"
ms.author: "wiwagn"
manager: "wpickett"
---
# null（C# 参考）
[!INCLUDE[vs2017banner](../../../csharp/includes/vs2017banner.md)]

`null` 关键字是表示不引用任何对象的 null 引用的文字值。  `null` 是引用类型变量的默认值。  普通值类型不能为 null。  但是，C\# 2.0 引入了可以为 null 值的类型。  请参见 [可以为 null 的类型](../../../visual-basic/reference/command-line-compiler/index.md)。  
  
 下面的示例演示 null 关键字的一些行为：  
  
 [!CODE [csrefKeywordsLiteral#1](../CodeSnippet/VS_Snippets_VBCSharp/csrefKeywordsLiteral#1)]  
  
## C\# 语言规范  
 [!INCLUDE[CSharplangspec](../../../csharp/language-reference/keywords/includes/csharplangspec_md.md)]  
  
## 请参阅  
 [C\# 参考](../../../visual-basic/reference/command-line-compiler/index.md)   
 [C\# 编程指南](../../../csharp/programming-guide/index.md)   
 [C\# 关键字](../../../csharp/language-reference/keywords/index.md)   
 [文字关键字](../../../csharp/language-reference/keywords/literal-keywords.md)   
 [默认值表](../../../csharp/language-reference/keywords/default-values-table.md)   
 [Nothing](../../../visual-basic/language-reference/nothing.md)
...
```  
 ...
 `delegate` 是一种可用于封装命名或匿名方法的引用类型。  委托类似于 C\+\+ 中的函数指针；但是，委托是类型安全和可靠的。  有关委托的应用，请参见[委托](../../../visual-basic/reference/command-line-compiler/index.md)和[泛型委托](../../../csharp/programming-guide/generics/generic-delegates.md)。  

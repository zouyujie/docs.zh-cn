...
返回[布尔](../../../csharp/language-reference/keywords/bool.md)值 `true` 以指示操作数为 true，否则返回 `false`。  
 ...
 类型不能直接重载条件逻辑运算符（[&&](../../../csharp/language-reference/operators/conditional-and-operator.md) 和 [&#124;&#124;](../Topic/%7C%7C%20Operator%20\(C%23%20Reference\).md)），但通过重载规则逻辑运算符和 `true` 与 `false` 运算符可以达到同样的效果。  
